...
<header class="ps-header ps-header--2 ps-header--8"> 
        ...
        <div class="modal-dialog modal-md">
          <div class="modal-content">
            ...
            <div class="modal-header">
                ...
                <button onclick="close_pomodal()" style="font-weight: bold;position: absolute;right: 10px;top: 4px;border: none;background-color: transparent;cursor: pointer;font-size: 22px;">&times;</button>
            </div>

            <div class="modal-body py-5 pl-5">
                    <form action="{{ route('checkoutstore') }}" method="POST" enctype="multipart/form-data" id="upload_po_form">
                        @csrf
                        <input type="hidden" name="purchaseno" id="purchaseno" value="{{ '' }}">
                        <input type="hidden" name="from_po_upload" id="from_po_upload" value="from_po_upload">
                        <input type="hidden" name="payment_type" id="payment_type" value="Purchase Order">
                        <label style="color: #2f5cbe !important;">Select PO</label>
                        <div class="row">
                            <div class="col-7 ">
                                <input type="file" required name="purchasedoc" id="po_file" accept="application/pdf">
                                <span class="text-muted" style="font-size: 14px;position: absolute;left: 19px;top: 45px;">-----Only PDF fromat acceptable-----</span>
                            </div>
                    ...
                    <div class="row pt-4">
                        <div class="col-7">
                            <!-- <p class="text-muted" style="font-size: 14px;">Only PDF fromat acceptable</p> -->
                        </div>
    ...
    function close_pomodal() {
        if(confirm("Do you want to really closed") == true){
            $("#upload_po_form")[0].reset();
            $("#uploadPOModal").modal('hide');
        }
    }

    function ClearForm() {
        if(confirm("Do you want to Cancel the file ?") == true){
            $("#upload_po_form")[0].reset();
            // $("#uploadPOModal").modal('hide');
        }
    }
